...
##**********Organize data according to Pedigree!!!!
str(eBee1Y_data)
...
##Matrix relationship A
ped<-read.csv("Matrix.A.csv",header=F,check.names = F) 
#ped<- as.matrix(ped)
# asreml.Ainverse then creates the inverse of the additive genetic relationship matrix
# from the pedigree data
ainv<-solve(ped)
dim(ainv)
eBee1Y_data$GID<-factor(eBee1Y_data$GID)
rownames(ainv) = levels(eBee1Y_data$GID)
colnames(ainv) = levels(eBee1Y_data$GID)
ainv
##loop across the traits
for(k in grep("PH_", colnames(eBee1Y_data))){ ## iterates through each of the plant height 
  ...
  mod<-asreml(fixed=pheno ~  REP + 1, 
              random = ~ ped(GID) + REP/SUB, 
              data=eBee1Y,  
              ginverse=list(GID=ainv), 
              maxiter=1000,
              singular.ok=TRUE) 
  
  h2a_mod<- pin(mod, h2a ~ V2/(V2 + (V4/2))) #H2a estimated
  summary(mod)
  summary(mod)$varcomp
...
write.csv(blups_all, "BLUPs_ped_A7_eBee_1Y.csv", row.names=F) ## writes the BLUEs into a CSV

anova.asreml(mod) #effect testing
